...
* Overview
  ...
  Said hypothetical person is intelligent, has an interest in programming, but is also a bit
...
*** =case=: Another form of conditional
...
*** DONE Guards: Yet /another/ form of conditionals
    CLOSED: [2019-02-20 Wed 20:25]
    ...
    So, you basically have "=|=", a boolean expression, and then "= theValue".  (There's no
...
*** The difference between pattern-matching and conditionals
    ...
    You've probably figured it out by now (or maybe you've started to get an inkling), but the difference between
    conditionals and pattern-matching is this:  with *conditionals*, you're using a boolean expression that can be as
    complex as you want but that distills down to either True or False; and with *pattern-matching*, you're checking the
...
*** Pulling pattern-matching and data types together
    ...
    that we're pattern-matching on the data type, and the underscore is both a wildcard and a
    statement that we don't care about the argument in that position (in the function invocation "=f
    a b=", the =a= and =b= are /arguments/ to the function).
    ...
    argument, we don't care what the first argument; we know the result is Nothing.
    ...
    do exactly that.  We pull the /a/ out of the first =Just a= argument (we use parentheses to keep
    Haskell from getting confused about what goes with what), and we pull the /b/ out of the second
    =Just b=, and then we jam them together with a =+= and put the result back inside another
    =Just=.
...
** TODO =Either=
** TODO =deriving= and common classes
