...
* IN-PROGRESS Data type constructors, pattern-matching, conditionals, and more function stuff
  ...
  are like agglomerations of other data.  For example, you could a data type of =Car=, with year,
  ...
  A cost is a number.  A loan insurance rate is a number.  A bank name is a string.  A check number
...
** A simple data type
   ...
   direction and returns the number of degrees that direction corresponds to?  Great, but what if we
   asked it direction "Esta" is?  ERROR.  We can avoid that by using this new data type instead.
   Here's the function signature ("signature": the list of input types and return values a
   function has).

   : bearing :: Direction -> Int

   So, the function takes a Direction and returns an Int (an integer).
   ...
   return value type, all separated by (short) arrows ("=->=").

** Implementation of the =bearing= function with conditionals (=if=)
   ...
   /someValue/.  Else, the entire expression the value /someOtherValue/.
...
** =case= statement
   ...
   So, that's prettier.  (And it works, because I extended the unit test with a little
   copy-and-paste.)

   (The =case= statement uses pattern-matching, by the way.  [That redundant sentence is redundant.])
...
** Pattern-matching
   ...
   What the heck?  This is /pattern-matching/.  There are /no conditionals/ here.  Conditionals are
   ok, but sometimes it's clearer to avoid them.  What's going on here is that we're saying
   (obviously, I guess) that if the function is called with the value =North=, the function's value
   is 0, and if it's called with the value =East=, its value is 90, and so on.

   You will definitely see more pattern-matching in the future, but it'll be a bit more complex
   (don't worry, not too crazy).

   Note that what's happening in the =case= statement is also pattern-matching, btw.
...
** =Maybe=
   ...
   So, we're saying that, like =Direction=, a =Maybe= value can one of two possible values:
   =Nothing= and =Just a=.

   But, wait, what's the "=a="?

   Well, that is Haskellish for "any old thing, of any old type".  (The "a" comes from the beginning
   of the alphabet, not the first letter of "any".)
   ...
   we return?  Should we just blow up the program?
